...
var initOmniture = function initOmniture(playerName)
{
	...
	s.linkTrackVars  = "events,eVar7,eVar32,prop22,prop23";
	s.linkTrackEvents = "event6,event7,event16,event17,event18,event21,event22";
	s.pageName = location;
	...
	s.visitorNamespace="nfl";
	s.trackingServer="metrics.nfl.com";
    s.trackingServerSecure="smetrics.nfl.com";
	...
	s.Media.monitor = function (s,media) {
		...
		var percent = parseInt(media.percent);
		...
		if (media.mediaEvent == "MONITOR")  {
					...
					if (fireRequest) {
						fireRequest = false;
						//trackInitialData(_lastStartData);
		...
		if (media.mediaEvent == 'OPEN') {
			...
			s.eVar32= media.playerName;
			s.prop22= media.playerName;
			s.prop23= "HTML5 ePlayer:" + media.playerName;
			s.ePlayer.setVariables();
			s.Media.track(media.name);
    ...
    {
		s.channel = s.ePlayer.data.channel;
		s.eVar7 = TS.Modules.PlaylistModel.title+"_"+customData.videoId;
	}

    s.ePlayer.setCustomData = function(p_data)
    {
		customData = p_data;
		s.channel 	= p_data.channelName;
		s.eVar7  	= TS.Modules.PlaylistModel.title+"_"+customData.videoId;
    }
