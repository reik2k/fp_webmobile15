...
var s_account           = "premiumtveplayerdev"
var s                   = null;
var firstVideoTracked   = false;


var initOmniture = function initOmniture(playerName)
{
	if(TS.Modules.ConfigModel.partner != '15724' )
    		{
				
     			s_account = "premiumtveplayermaster";
   			}
    		
			else {
				
      			s_account = 'premiumtveplayertestsuite2011';
    		}
	...
	s.linkTrackVars  = "events,prop1,prop2,prop3,prop4,prop5,prop6,prop7,prop8,prop9,prop10,prop11,prop12,prop13,prop14,prop18,prop32,prop33,prop35,prop36,prop37";
	s.linkTrackVars += "eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,eVar11,eVar12,eVar13,eVar14,eVar15,eVar16,eVar17,eVar18,eVar21,eVar22,eVar23,eVar24,eVar25,eVar26,eVar27,eVar28,eVar29,eVar30,eVar31,eVar32,eVar33,eVar34,eVar35,eVar36,eVar37,eVar38,eVar39,eVar40,eVar41,eVar42,eVar43,eVar44,eVar45,eVar46";
	s.linkTrackEvents = "event1,event2,event3,event4,event5,event6,event7,event8";
	
	/* WARNING: Changing any of the below variables will cause drastic
	changes to how your visitor data is collected.  Changes should only be
	made when instructed to do so by your account manager.*/
	s.visitorNamespace="premiumtv";
	s.trackingServer="metrics.performgroup.com";
    s.trackingServerSecure="smetrics.performgroup.com";
	...
	s.Media.trackEvents = s.linkTrackEvents;
	s.Media.playerName = playerName;
	s.Media.trackUsingContextData = true;
	s.Media.contextDataMapping = {
		"a.media.name"			:"prop3,eVar3",
		"a.media.segment"		:"eVar1,prop5",
		"a.media.length"		:"prop14,eVar16",
		"a.contentType"			:"eVar2",
		"a.media.timePlayed"	:"event1",
		"a.media.view"			:"event2",
		"a.media.segmentView"	:"event4",
		"a.media.complete"		:"event3",
		"a.media.playerName"    :"prop8"
	};
	
	s.Media.monitor = function (s,media) {
		if (media.mediaEvent == 'OPEN')
        {
			if (s.ePlayer.data.autoPlay && s.ePlayer.data.firstOpen)
            {
				s.events = s.events ? s.events+',event5' : 'event5';
				s.ePlayer.data.firstOpen = false;
			}
			s.ePlayer.setVariables();
			s.eVar18 = s.ePlayer.data.videoType;
			s.Media.track(media.name);
			s.manageVars("clearVars","pageName",2);
		}
	} 
	...
	s.ePlayer.manageRights = function() {
	
		if(TS.Modules.PlaylistModel.rights.right!=null && TS.Modules.PlaylistModel.rights.right!="undefined")
		{
			if(Object.getPrototypeOf(TS.Modules.PlaylistModel.rights.right)=== Object.prototype)
			{
				s.eVar41 = 1;
				s.eVar42 = TS.Modules.PlaylistModel.rights.right.name;
				s.eVar43 = TS.Modules.PlaylistModel.rights.right.id;
			}
			
			else
			{
				s.eVar41 = TS.Modules.PlaylistModel.rights.right.length;
				s.eVar42 = TS.Modules.PlaylistModel.rights.right[0].name;
				s.eVar43 = TS.Modules.PlaylistModel.rights.right[0].id;
				s.eVar44 = TS.Modules.PlaylistModel.rights.right[1].name;
				s.eVar45 = TS.Modules.PlaylistModel.rights.right[1].id;
			}
		}
	
	}
	
	s.ePlayer.setVideoType = function(videoType) {
		s.ePlayer.data.videoType = videoType;
	}
	
	s.ePlayer.setVariables = function()
    {
		s.prop1  = s.eVar4  = s.ePlayer.data.lang;
		s.prop2  = s.eVar5  = s.ePlayer.data.autoPlay ? 'auto-play' : 'manual';
		s.prop6  = s.eVar6  = s.ePlayer.data.channel;
		s.prop7  = s.eVar7  = s.ePlayer.data.partner;
		s.prop9  = s.eVar8  = s.ePlayer.data.playerSize;
		s.prop10 = s.eVar11 = s.ePlayer.data.country;
		s.prop12 = s.eVar10 = 'HTML5';
		s.prop13 = s.eVar15 = s.ePlayer.data.fullScreen ? 'full-screen' : 'embedded';
		s.prop18 = s.eVar12 = s.ePlayer.data.url;
		s.prop36 = s.eVar14 = s.ePlayer.data.playerType;
		s.prop4  = s.eVar17 = s.ePlayer.data.playerVersion;
		s.eVar21 = s.ePlayer.data.country;
		s.prop37 = "not-scrollToPlay";
        s.eVar31 = s.ePlayer.data.startMute;
        s.eVar38 = TS.Modules.ChannellistModel.id;
        s.eVar39 = TS.Modules.PlaylistModel.id;
        s.eVar40 = TS.Modules.ConfigModel.playerId;
        s.eVar46 = TS.Modules.ConfigModel.windowLocationFull;
		
		s.ePlayer.manageRights();		
	}

    s.ePlayer.setCustomData = function(p_data)
    {
        s.prop6  = s.eVar6  = s.ePlayer.data.channel = p_data.channel;
        s.eVar23 	= p_data.genre;
        s.eVar24 	= p_data.contentIndex;
        s.eVar25 	= p_data.contentKeywords;
        s.eVar26 	= p_data.contentType;
        s.eVar27 	= p_data.contentDuration;
        s.eVar28 	= p_data.videoDuration;
        s.eVar29 	= ((new Date()).getTimezoneOffset()) / 60;
        s.eVar30 	= (new Date()).getDay();
        s.eVar32 	= p_data.windowLocation;
        s.eVar33    = p_data.videoIsStartedByClick;
		s.eVar34	= p_data.nextVideoItem;
		s.eVar35	= p_data.nextVideoItem2;
		s.eVar36	= p_data.nextVideoItem3;
		s.eVar37	= p_data.internalReferrer;
        s.eVar38	= p_data.channelId;
        s.eVar39	= p_data.videoId;
        s.eVar40	= p_data.playerId;
			
		s.ePlayer.manageRights();		
    }
	
	s.ePlayer.init = function(lang, autoPlay, channel, partner, playerSize, country, fullScreen, url, playerType, playerVersion, startMute) {
		
		s.ePlayer.data                  = {};
		s.ePlayer.data.lang             = lang;
		s.ePlayer.data.autoPlay         = autoPlay;
		s.ePlayer.data.channel          = channel;
		s.ePlayer.data.partner          = partner;
		s.ePlayer.data.playerSize       = playerSize;
		s.ePlayer.data.country          = country;
		s.ePlayer.data.fullScreen       = fullScreen;
		s.ePlayer.data.url              = url;
		s.ePlayer.data.playerType       = playerType;
		s.ePlayer.data.playerVersion    = playerVersion;
		s.ePlayer.data.videoType        = '';
		s.ePlayer.data.firstOpen        = true;
        s.ePlayer.data.startMute        = startMute;

		if (!autoPlay) {
			s.ePlayer.setVariables();
			s.events = 'event5';
			s.tl(true,'o','HTML5 ePlayer loaded');
			s.manageVars("clearVars","pageName",2);
		}

        var eventData = {};

        eventData.data      = s.ePlayer.data;
        eventData.events    = s.ePlayer.events;
        eventData.account   = s_account;

        E.trigger(E.EVENT_OMNITURE_INIT, s.ePlayer, eventData);
	}

    s.ePlayer.trackFirstVideoStarted = function()
    {
        if(!firstVideoTracked)
        {
            s.ePlayer.setVariables();
            s.events = 'event7';
            s.tl(true,'o','HTML5 ePlayer first video started');
            s.manageVars("clearVars","pageName",2);

            firstVideoTracked = true;
        }
    }
...
return{
	...
	initOmniture:function(value)
	{
		initOmniture(value);
	},
	initEplayer:function(lang, autoPlay, channel, partner, playerSize, country, fullScreen, url, playerType, playerVersion, startMute)
	{
		s.ePlayer.init(lang, autoPlay, channel, partner, playerSize, country, fullScreen, url, playerType, playerVersion, startMute);
	},
	
	open:function(title,duraction,partner)
	{
        s.ePlayer.trackFirstVideoStarted();
        s.Media.open(title,duraction,partner);
	},
	
	play:function(title,time)
	{
		s.Media.play(title,time);
	},
	...
	{
		s.ePlayer.setVideoType(videotype_content);
	},
	
	setCustomData:function(p_data)
	{
		s.ePlayer.setCustomData(p_data);
	} ,
    ...
    trackFullSizeChange : function()
    {
        s.ePlayer.setVariables();
        s.events = 'event8';
        s.eVar22 = "" + TS.Modules.initialize.fullSizeOn;
        s.tl(true,'o','ePlayer switched fullsize on/off.');
        s.manageVars("clearVars","pageName",2);
    }
};
}